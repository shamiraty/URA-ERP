                    ...
                    <div class="dropdown dropdown-large">
                        <button class="has-indicator w-40-px h-40-px bg-neutral-200 rounded-circle d-flex justify-content-center align-items-center position-relative" type="button" data-bs-toggle="dropdown">
                            ...
                            @if($notifications->where('is_read', false)->count() > 0)
                                <span class="badge bg-danger rounded-circle position-absolute top-0 end-0">{{ $notifications->where('is_read', false)->count() }}</span>
                            @endif
                        </button>
                        <div class="dropdown-menu to-top dropdown-menu-lg p-0">
                        <div class="m-3 py-3 px-4 rounded bg-primary bg-opacity-10 mb-4 d-flex align-items-center justify-content-between gap-2 shadow-sm">
    <div>
    ...
    <span class="text-white fw-bold bg-primary rounded-circle d-flex justify-content-center align-items-center" 
          style="width: 35px; height: 35px; font-size: 0.85rem; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
    ...
    @forelse($notifications as $notification)
        <a class="dropdown-item d-flex align-items-center p-2 border-bottom" href="javascript:;" style="transition: background-color 0.3s ease;">
            <div class="notify bg-light-info  rounded-circle p-1 me-2">
                <iconify-icon icon="bitcoin-icons:verify-outline" class="fs-6"></iconify-icon>
            </div>
            <div class="flex-grow-1">
                <small class="msg-name mb-1 fw-semibold text-dark">{{ $notification->message }}</small>
                <br>
                <small class="msg-time text-muted mb-0">{{ $notification->created_at->diffForHumans() }}</small>
            </div>
            @if(!$notification->is_read)
                <form action="{{ route('notifications.read', $notification->id) }}" method="POST" class="ms-auto">
                    @csrf
                    <button type="submit" class="btn btn-sm btn-link p-0 text-success" style="font-size: 0.8rem;">
